...
- 画面はライトモード専用です。デザインの方向は「Linear の静けさ + IDE のデータグリッド」。
  トークンは `src/styles.css` の先頭にまとまっています。
  - 面はほぼ白（背景 `#fbfbfc` / パネル `#ffffff`）、線は薄いヘアライン、影は1pxぶんだけ。
  - アクセントはインディゴ `#5b5bd6` の1色だけ。現在地（タブの下線）と主操作にしか使いません。
  - 状態は淡い地 + 濃い文字（緑＝済 / 黄＝要対応 / 赤＝不足）。地の色だけで意味を運ばせません。
  - 二次・三次の文字色は白地でコントラスト 5.0:1 / 4.7:1 を確保しています（11px のラベルまで読めるように）。
  - 書体は Inter（UI）と JetBrains Mono（表の数値・日付）。どちらも Latin サブセットだけを
    `public/fonts/` に自前ホストしています（合計約80KB、OFL）。日本語は端末のゴシック体に任せます。
- スプリントカードの「終了条件」ゲートだけが強調要素です。立替の精算がどこまで進んだかと、
  終了条件（残高がプラス / 未精算ゼロ）の達成状況を1行で示します。
- グラフは外部ライブラリを使わず SVG を直接描いています（`src/charts/`）。
  収入 = インディゴ `#5b5bd6` / 支出 = 橙 `#eb6834` の2色は、色覚特性を含む見分けやすさを確認した組み合わせです。
